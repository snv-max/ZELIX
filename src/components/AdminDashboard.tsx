...
export function AdminDashboard({ defaultTab = 'overview' }: { defaultTab?: 'overview' | 'products' | 'categories' | 'orders' | 'customers' }) {
  ...
  // Active Tab
  const [activeTab, setActiveTab] = useState<'overview' | 'products' | 'categories' | 'orders' | 'customers'>(defaultTab);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <nav className="lg:col-span-3 space-y-2">
            {[
              ...
              { id: 'products', label: 'Garments CRUD', icon: ShoppingBag, path: '/admin/products' },
              { id: 'categories', label: 'Categories', icon: FolderOpen, path: '/admin' }, // Categories can load overview with tab switch or remain on admin
              { id: 'orders', label: 'Global Orders', icon: ClipboardList, path: '/admin/orders' },
              { id: 'customers', label: 'Customers', icon: Users, path: '/admin' },
            ].map((tab) => {
              const Icon = tab.icon;
              const isTabActive = activeTab === tab.id;
              
              // We handle clicks locally, or navigate if clicking subroutes
              const handleClick = () => {
              ...
              return (
                <button
                  key={tab.id}
                  onClick={handleClick}
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded text-sm uppercase tracking-widest font-semibold transition-all ${isTabActive ? 'bg-white text-black font-extrabold' : 'text-muted-foreground hover:text-white hover:bg-white/5'}`}
                >
