...
function SignupContent() {
  ...
  const handleMockSignup = (e: React.FormEvent) => {
    ...
    // Save temporary details in localStorage to use after OTP completes
    localStorage.setItem('zelix_temp_email', email);
    localStorage.setItem('zelix_temp_name', fullName);

    setTimeout(() => {
      setLoading(false);
      // Redirect to custom OTP verification screen
      router.push(`/otp-verification?redirect=${encodeURIComponent(redirectUrl)}`);
    }, 800);
  };

  // 1. Render Clerk SignUp if Clerk is active
  ...
  // 2. Render Mock Signup for local auditing
  return (
          ...
          <form onSubmit={handleMockSignup} className="space-y-5">
            ...
            <div>
              ...
              <div className="relative">
                <input 
                  ...
                  placeholder="e.g. member@zelix.com"
                  className="w-full bg-[#18181b]/50 border border-border text-sm text-white placeholder-border/50 rounded pl-10 pr-3 py-2.5 focus:outline-none focus:border-white transition-colors"
                />
            ...
            <div>
              ...
              <input 
                ...
                placeholder="••••••••"
                className="w-full bg-[#18181b]/50 border border-border text-sm text-white placeholder-border/50 rounded px-3.5 py-2.5 focus:outline-none focus:border-white transition-colors"
              />
            </div>
            ...
            <button 
              type="submit"
              disabled={loading}
              className="w-full h-12 bg-white text-black font-extrabold text-xs uppercase tracking-widest rounded hover:bg-zinc-200 transition-colors flex items-center justify-center gap-2 cursor-pointer disabled:bg-zinc-800 disabled:text-zinc-500 mt-6"
            >
              {loading ? 'Registering Details...' : (
                <>
